...
-------------------------------------------------------------------------------
SELECT * FROM hospital_management.prescriptions;
SELECT * FROM hospital_management.patients;
SELECT * FROM hospital_management.departments;
SELECT * FROM hospital_management.hospitals;
SELECT * FROM hospital_management.medications;
...
-- Queries Part 1 (3 points each):
-- Find all patients born after the year 2000
SELECT *
FROM patients
WHERE YEAR(patient_dob) > 2000;
...
SELECT MONTH(appointment_date) AS Month_Number, COUNT(*) AS Total_Appointments
FROM appointments
...
SELECT *
FROM medications
WHERE medication_description LIKE "%Pain%" OR "%Infection%";

------------------------------------------------------------------------------------------

-- Show all doctors with their hospital name
SELECT doctors.doctor_name, doctors.doctor_speciality, hospitals.hospital_name
FROM doctors
JOIN hospitals ON hospitals.hospital_id = doctors.hospital_id;

-----------------------------------------------------------------------------------------------------------

-- Find the names, phones and appointment_dates of all patients with appointments in August 2025
SELECT patients.patient_name, patients.patient_phone, appointments.appointment_date
FROM patients
JOIN appointments ON patients.patient_id = appointments.patient_id
WHERE MONTH(appointment_date) = 8
AND YEAR(appointment_date) = 2025; 

-------------------------------------------------------------------------------------------------------------

-- Show all room numbers and their capacities from the Neurology departments
SELECT rooms.room_no, rooms.room_capacity, departments.department_name
FROM rooms
JOIN departments ON rooms.department_id = departments.department_id
WHERE department_name = "Neurology";

----------------------------------------------------------------------------------------------------------------

-- Count how many doctors work in each hospital
SELECT COUNT(doctors.doctor_id) AS doctor_id, hospitals.hospital_name
FROM hospitals
...
SELECT  COUNT(appointments.appointment_id) AS Total_Appointments, patients.patient_name
FROM patients
JOIN appointments ON patients.patient_id = appointments.patient_id
...
SELECT patients.patient_name, doctors.doctor_name, appointments.appointment_id, appointments.appointment_date,appointments.reason
FROM appointments
JOIN doctors ON appointments.doctor_id = doctors.doctor_id
JOIN patients ON patients.patient_id = appointments.patient_id;

--------------------------------------------------------------------------
-- Queries Part 2 (5 points each)
...
SELECT appointments.reason, patients.patient_name, patients.patient_dob,
	CASE
		WHEN TIMESTAMPDIFF ( YEAR,patients.patient_dob, CURDATE()) <= 18 THEN "Pediatric"
        WHEN TIMESTAMPDIFF (YEAR,patients.patient_dob, CURDATE()) BETWEEN 19 AND 64 THEN "Adult"
        ELSE "Geriatric"
        END AS age_group
From patients
JOIN appointments ON patients.patient_id = appointments.patient_id
WHERE appointments.reason ="Emergency";
...
FROM departments
JOIN hospitals ON hospitals.hospital_id = departments.hospital_id
WHERE hospitals.hospital_name = "Green Valley Medical Center";

------------------------------------------------------------------------------------------------------------

-- Find patients who have never had a prescription
SELECT prescriptions.patient_id,prescriptions.prescription_id, patients.patient_name
FROM patients
LEFT JOIN prescriptions ON patients.patient_id = prescriptions.patient_id
HAVING prescriptions.prescription_id is null;

-----------------------------------------------------------------------------------------------------------

-- List name, address and phone for patients who have appointments in more than one hospital
SELECT patients.patient_name, patients.patient_address, patients.patient_phone, patients.patient_id
FROM appointments
JOIN patients ON patients.patient_id = appointments.patient_id
JOIN doctors ON doctors.doctor_id = appointments.doctor_id
GROUP BY patients.patient_name, patients.patient_address, patients.patient_phone, patients.patient_id
HAVING COUNT(DISTINCT doctors.hospital_id) > 1;

----------------------------------------------------------------------------------------------------------
-- Show the latest appointment for each patient
SELECT patients.patient_id, patients.patient_name, MAX(appointments.appointment_date) AS Latest_Appiontment
FROM appointments
JOIN patients ON patients.patient_id = appointments.patient_id
GROUP BY patients.patient_id, patients.patient_name;
...
WITH Medication_Frequency AS (
	SELECT COUNT(prescriptions.medication_id) AS Total_prescribed, medications.medication_name,
    DENSE_RANK() OVER (ORDER BY COUNT(prescriptions.medication_id) DESC) AS Rnk
FROM medications
JOIN prescriptions ON prescriptions.medication_id = medications.medication_id
GROUP BY medications.medication_id, medications.medication_name
)
SELECT medication_name, Total_prescribed
...
WITH HospitalDoctorCounts AS (
    SELECT 
        ...
        COUNT(doctors.doctor_id) AS total_doctors,
        RANK() OVER (ORDER BY COUNT(doctors.doctor_id) ASC) as Rnk
    FROM hospitals
    JOIN doctors ON hospitals.hospital_id = doctors.hospital_id
    GROUP BY hospitals.hospital_id, hospitals.hospital_name
)
SELECT hospital_name, total_doctors
...
WITH Largest_Room AS (
	SELECT SUM(rooms.room_capacity) AS Second_largest_Capacity, departments.department_name,hospitals.hospital_name,
    DENSE_RANK()  OVER (PARTITION BY (hospitals.hospital_id) ORDER BY SUM(rooms.room_capacity)  DESC) AS Rnk
FROM departments
JOIN hospitals ON hospitals.hospital_id = departments.hospital_id
JOIN rooms ON rooms.department_id = departments.department_id
GROUP BY hospitals.hospital_id, hospitals.hospital_name, departments.department_id, departments.department_name
)
SELECT hospital_name, department_name, Second_largest_Capacity
FROM Largest_Room
WHERE Rnk = 2;
